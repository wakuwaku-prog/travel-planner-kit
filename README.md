...
- **路线图 => 高德 App**：下载网站「导入路线」面板的 **KML** 文件 → 高德 App「我的 → 收藏 → 导入」。导入后成为收藏/足迹里的点与路线，可直接规划导航。
- **一键导航**：每个点位有「🧭 高德导航」按钮（URL Scheme 唤起 App），手机上即点即用；每日另有起终点整体路线链接 + 二维码。
- **GPX** 供两步路 / 六只脚 / 运动软件等通用导入。
- 坐标来自高德（GCJ-02）：导入高德系 App 无偏移；导入 Google 地球等 WGS84 工具会有系统偏差，属正常坐标差异。
- 高德官方不支持第三方直接把行程写入用户收藏，KML 中转是目前通用做法。
